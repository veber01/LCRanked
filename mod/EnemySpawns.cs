...
namespace LCRanked
{
    public static class DeterministicEnemyPlanner
    {
        ...
        private static readonly AccessTools.FieldRef<RoundManager, int> enemyRushIndexRef =
            AccessTools.FieldRefAccess<RoundManager, int>("enemyRushIndex");
        ...
        public static void PlanEnemySpawnsForWholeDay(RoundManager rm)
        {
            ...
            int enemyRushIndex = enemyRushIndexRef(rm);
            ...
            Plugin.Log.LogInfo($"[LCRanked] Deterministic enemy plan built: {plannedSpawns.Count} enemies planned across {batchCumulativeCounts.Count} batches.");
            LogFullDayPlan(rm);
        }
        ...
        private static void LogFullDayPlan(RoundManager rm)
        {
            ...
            for (int i = 0; i < plannedSpawns.Count; i++)
            {
                ...
                Plugin.Log.LogInfo($"[LCRanked] Plan #{i} (batch {batch}): {enemyType.enemyName} @ t={planned.spawnTime:F1}");
            }
        }
        ...
        public static void SpawnReadyVents(RoundManager rm)
        {
            for (int i = 0; i < rm.allEnemyVents.Length; i++)
            {
                ...
                {
                    Plugin.Log.LogInfo($"[LCRanked] Found enemy vent which has its time up: {rm.allEnemyVents[i].gameObject.name}. Spawning {rm.allEnemyVents[i].enemyType.enemyName} from vent.");
