...
public class Terminal : NetworkBehaviour
{
	...
	private string TextPostProcess(string modifiedDisplayText, TerminalNode node)
	{
		...
		if (modifiedDisplayText.Contains("[buyableItemsList]"))
		{
			...
			else
			{
				...
				{
					stringBuilder2.Append("\n* " + buyableItemsList[k].itemName + "  //  Price: $" + (int)((float)buyableItemsList[k].creditsWorth * ((float)itemSalesPercentages[k] / 100f)));
		...
		if (modifiedDisplayText.Contains("[unlockablesSelectionList]"))
		{
			...
			else
			{
				...
				{
					stringBuilder5.Append($"\n{ShipDecorSelection[n].creatureName}  //  Price: ${ShipDecorSelection[n].itemCost}");
				}
		...
		if (modifiedDisplayText.Contains("[scanForItems]"))
		{
			...
			{
				if (!array[num6].itemProperties.isScrap)
				{
					continue;
				}
				if (!StartOfRound.Instance.inShipPhase)
				{
					if (!array[num6].isInShipRoom && !array[num6].isInElevator)
					{
				...
				{
					num4 += array[num6].scrapValue;
					num3++;
				}
			}
			modifiedDisplayText = ((!StartOfRound.Instance.inShipPhase) ? modifiedDisplayText.Replace("[scanForItems]", $"There are {num3} objects outside the ship, totalling at an approximate value of ${num4}.") : modifiedDisplayText.Replace("[scanForItems]", $"There are {num3} objects in the ship, totalling at ${num4}."));
		}
	...
	private void LoadNewNodeIfAffordable(TerminalNode node)
	{
		...
		if (node.shipUnlockableID != -1)
		{
			...
			{
				if (StartOfRound.Instance.shipIsLeaving || StartOfRound.Instance.beganLoadingNewLevel || RoundManager.Instance.dungeonIsGenerating || (!StartOfRound.Instance.inShipPhase && !StartOfRound.Instance.shipHasLanded))
				{
					LoadNewNode(terminalNodes.specialNodes[28]);
					return;
				}
	...
	public void BuyVehicleServerRpc(int vehicleID, int newGroupCredits, bool useWarranty = false)
	{
		...
		{
			__rpc_exec_stage = __RpcExecStage.Send;
			if (newGroupCredits <= groupCredits)
			{
	...
	{
		__registerRpc(1713627637u, __rpc_handler_1713627637, "PlayTerminalAudioServerRpc");
		__registerRpc(1118892272u, __rpc_handler_1118892272, "PlayTerminalAudioClientRpc");
		__registerRpc(2452398197u, __rpc_handler_2452398197, "BuyVehicleServerRpc");
		__registerRpc(3926234553u, __rpc_handler_3926234553, "BuyVehicleClientRpc");
		__registerRpc(4003509079u, __rpc_handler_4003509079, "BuyItemsServerRpc");
		__registerRpc(3085407145u, __rpc_handler_3085407145, "SyncGroupCreditsServerRpc");
		__registerRpc(2039928764u, __rpc_handler_2039928764, "SyncGroupCreditsClientRpc");
		__registerRpc(1261428289u, __rpc_handler_1261428289, "SyncTerminalValuesServerRpc");
		__registerRpc(1505747100u, __rpc_handler_1505747100, "SyncTerminalValuesClientRpc");
		__registerRpc(4047492032u, __rpc_handler_4047492032, "SetTerminalInUseServerRpc");
		__registerRpc(517264497u, __rpc_handler_517264497, "KickOffTerminalClientRpc");
		__registerRpc(2420057819u, __rpc_handler_2420057819, "SetTerminalInUseClientRpc");
