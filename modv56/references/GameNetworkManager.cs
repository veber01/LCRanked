...
public class GameNetworkManager : MonoBehaviour
{
	...
	{
		if ((type != LogType.Exception && type != LogType.Error) || HUDManager.Instance == null || localPlayerController == null)
		{
			return;
		}
	...
	private void ConnectionApproval(NetworkManager.ConnectionApprovalRequest request, NetworkManager.ConnectionApprovalResponse response)
	{
		...
		if (flag)
		{
			string text = Encoding.ASCII.GetString(request.Payload);
	...
	private void Singleton_OnClientDisconnectCallback(ulong clientId)
	{
		...
		else if ((clientId == 0L || clientId == NetworkManager.Singleton.LocalClientId) && NetworkManager.Singleton.IsClient)
		{
			Instance.disconnectReason = 1;
			Instance.Disconnect();
			return;
		}
	...
	private void SteamMatchmaking_OnLobbyCreated(Result result, Lobby lobby)
	{
		...
		try
		{
			switch (ModdedCheck.GetModdedState())
			{
			case ModdedState.Modded:
				lobby.SetData("dmods", "t");
				break;
			case ModdedState.Unknown:
				lobby.SetData("dmods", "u");
				break;
			default:
				lobby.SetData("dmods", "f");
				break;
			}
		}
		catch (Exception arg)
		{
			Debug.Log($"Error when trying to get modded state: {arg}");
			lobby.SetData("dmods", "u");
		}
	...
	public void ResetSavedGameValues()
	{
		...
		{
			ES3.Save($"Level{startOfRound.levels[i].levelID}Mold", 0, currentSaveFileName);
			ES3.Save($"Level{startOfRound.levels[i].levelID}MoldOrigin", -1, currentSaveFileName);
	...
	private void SaveGameValues()
	{
		...
		if (!StartOfRound.Instance.inShipPhase)
		{
			...
			int num = ES3.Load($"Level{startOfRound.currentLevel.levelID}TimesSavescumming", currentSaveFileName, 0);
			ES3.Save($"Level{StartOfRound.Instance.currentLevel.levelID}TimesSavescumming", num + 1, currentSaveFileName);
			if (startOfRound != null && num >= 4 && StartOfRound.Instance.currentLevel.canSpawnMold)
			{
				if (startOfRound.currentLevel.moldSpreadIterations <= 0)
				{
				...
				ES3.Save($"Level{startOfRound.currentLevel.levelID}Mold", startOfRound.currentLevel.moldSpreadIterations, currentSaveFileName);
				ES3.Save($"Level{startOfRound.currentLevel.levelID}MoldOrigin", startOfRound.currentLevel.moldStartPosition, currentSaveFileName);
			}
		}
		else
		{
			if (StartOfRound.Instance.beganLoadingNewLevel || RoundManager.Instance.dungeonIsGenerating)
			{
				return;
			}
			try
			{
				...
				if (timeOfDay != null)
				{
					...
					ES3.Save("ShownAdThisQuota", timeOfDay.hasShownAdThisQuota, currentSaveFileName);
				}
				...
				{
					ES3.Save("LuckyFurniture", timeOfDay.furniturePlacedAtQuotaStart.ToArray(), currentSaveFileName);
				}
				Terminal terminal = UnityEngine.Object.FindObjectOfType<Terminal>();
				if (terminal != null)
				{
					...
					ES3.Save("VehicleWarrantyTicket", terminal.hasWarrantyTicket, currentSaveFileName);
				}
				...
				{
					for (int i = 0; i < startOfRound2.levels.Length; i++)
					{
						ES3.Save($"Level{startOfRound2.levels[i].levelID}Mold", startOfRound2.levels[i].moldSpreadIterations, currentSaveFileName);
						ES3.Save($"Level{startOfRound2.levels[i].levelID}MoldOrigin", startOfRound2.levels[i].moldStartPosition, currentSaveFileName);
						if (moldSpreadManager != null)
						{
							if (moldSpreadManager.planetMoldStates[i].destroyedMold.Count > 0)
							{
								ES3.Save($"Level{startOfRound2.levels[i].levelID}DestroyedMold", moldSpreadManager.planetMoldStates[i].destroyedMold.ToArray(), currentSaveFileName);
							}
							else
							{
								ES3.DeleteKey($"Level{startOfRound2.levels[i].levelID}DestroyedMold", currentSaveFileName);
							}
						}
					}
					List<int> list = new List<int>();
					for (int j = 0; j < startOfRound2.unlockablesList.unlockables.Count; j++)
					{
						if (startOfRound2.unlockablesList.unlockables[j].hasBeenUnlockedByPlayer || startOfRound2.unlockablesList.unlockables[j].hasBeenMoved || startOfRound2.unlockablesList.unlockables[j].inStorage)
						{
							list.Add(j);
						}
						if (startOfRound2.unlockablesList.unlockables[j].IsPlaceable)
						{
							if (startOfRound2.unlockablesList.unlockables[j].canBeStored)
							{
								ES3.Save("ShipUnlockStored_" + startOfRound2.unlockablesList.unlockables[j].unlockableName, startOfRound2.unlockablesList.unlockables[j].inStorage, currentSaveFileName);
							}
							if (startOfRound2.unlockablesList.unlockables[j].hasBeenMoved)
							{
								ES3.Save("ShipUnlockMoved_" + startOfRound2.unlockablesList.unlockables[j].unlockableName, startOfRound2.unlockablesList.unlockables[j].hasBeenMoved, currentSaveFileName);
								ES3.Save("ShipUnlockPos_" + startOfRound2.unlockablesList.unlockables[j].unlockableName, startOfRound2.unlockablesList.unlockables[j].placedPosition, currentSaveFileName);
								ES3.Save("ShipUnlockRot_" + startOfRound2.unlockablesList.unlockables[j].unlockableName, startOfRound2.unlockablesList.unlockables[j].placedRotation, currentSaveFileName);
							}
						}
					}
					if (list.Count > 0)
					...
					int num2 = -1;
					if (StartOfRound.Instance.attachedVehicle != null)
					{
						num2 = StartOfRound.Instance.attachedVehicle.vehicleID;
					}
					ES3.Save("AttachedVehicleID", num2, currentSaveFileName);
					Debug.Log($"saved vehicle: {num2}");
					ES3.Save("DeadlineTime", (int)Mathf.Clamp(timeOfDay.timeUntilDeadline, 0f, 99999f), currentSaveFileName);
					ES3.Save("RandomSeed", startOfRound2.randomMapSeed, currentSaveFileName);
					ES3.Save("Stats_DaysSpent", startOfRound2.gameStats.daysSpent, currentSaveFileName);
					ES3.Save("Stats_Deaths", startOfRound2.gameStats.deaths, currentSaveFileName);
					ES3.Save("Stats_ValueCollected", startOfRound2.gameStats.scrapValueCollected, currentSaveFileName);
					ES3.Save("Stats_StepsTaken", startOfRound2.gameStats.allStepsTaken, currentSaveFileName);
				}
				Debug.Log($"SAVE ITEMS IN SHIP CALLED! inShipPhase: {StartOfRound.Instance.inShipPhase}");
				SaveItemsInShip();
			}
			catch (Exception arg)
			{
